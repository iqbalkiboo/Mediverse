...
export const mapListDiscountMedpoint = (data: any[]) => {
  return isArray(data)
    ? data?.map((item: any) => {
        return {
          id: item?.id,
...
export const mapDetailDiscountMedpoint = (data: any) => {
  return {
    id: data?.id,
    name: data?.name || '-',
    ...
    discountType:
      data?.discount_type === 'nominal' ? 'Nominal' : 'Persentase' || '-',
    ...
    discountPercentage:
      `${data?.discount_type === 'percentage' ? data?.amount : 0 || 0} %` ||
      '-',
